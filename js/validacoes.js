...
function removeSpecialCharSimpleSoLetras(strToReplace) {
...
function removeSpecialCharSimpleSoNumeros(strToReplace) {
    strSChar = 'áàãâäéèêëíìîïóòõôöúùûüçÁÀÃÂÄÉÈÊËÍÌÎÏÓÒÕÖÔÚÙÛÜÇ';
    strNoSChars = 'aaaaaeeeeiiiiooooouuuucAAAAAEEEEIIIIOOOOOUUUUC';
    var newStr = '';
    for (var i = 0; i < strToReplace.length; i++) {
...
function validar(dom, tipo) {
    switch (tipo) {
        case'text':
            //var regex = /[A-Za-z]/g;
            var regex = /^[\\0-9\-\\\/\=\^\+\~\´\`\;\:\>\<\,\|\.\_\*\&\¨\%\$\#\@\!\{\}\(\)\[\]\?\ÇçéúíóáÉÚÍÓÁèùìòàçÇÈÙÌÒÀõãñÕÃÑêûîôâÊÛÎÔÂëÿüïöäËYÜÏÖÄ\s]+$/;
            break;
        case'num':
            regex = /^[\\a-zA-Z\-\\\/\=\^\+\~\´\`\;\:\>\<\,\|\.\_\*\&\¨\%\$\#\@\!\{\}\(\)\[\]\?\ÇçéúíóáÉÚÍÓÁèùìòàçÇÈÙÌÒÀõãñÕÃÑêûîôâÊÛÎÔÂëÿüïöäËYÜÏÖÄ\s]/g;
            break;
    }
...
function validaCampos()
{
    ...
    if (document.getElementById('codCarga').value.length == regex) {
        alert('Informe o código da carga!');
        document.getElementById('codCarga').focus();
        return;
    }
    ...
    if (document.getElementById('pesoMercadoria').value.length == regex) {
        alert('Informe o peso da mercadoria!');
        document.getElementById('pesoMercadoria').focus();
        return;
    }
    if (document.getElementById('valorMercadoria').value.length == regex) {
        alert('Informe o valor da mercadoria!');
        document.getElementById('valorMercadoria').focus();
        return;
    }
    
    if (document.getElementById('quantidade').value.length == regex) {
